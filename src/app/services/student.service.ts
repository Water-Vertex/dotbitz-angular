...
export class StudentService {
  ...
  registerStudent(data: RegistrationRequest): Observable<any> {
    return this.http
      .post(`${this.StudentApiUrl}/register`, data)
      .pipe(catchError((err) => throwError(() => err)));
  }

  checkEmailExists(email: string): Observable<{ exists: boolean }> {
    return this.http
      .get<{ exists: boolean }>(`${this.StudentApiUrl}/check-email/${email}`)
      .pipe(catchError((err) => throwError(() => err)));
  }

  checkUsernameExists(username: string): Observable<{ exists: boolean }> {
    return this.http
      .get<{ exists: boolean }>(`${this.StudentApiUrl}/check-username/${username}`)
      .pipe(catchError((err) => throwError(() => err)));
  }

  // ---------------- Authenticated ----------------
  getStudentById(id: number): Observable<{ success: boolean; data: Student }> {
    return this.http
      .get<{
        success: boolean;
        data: Student;
      }>(`${this.StudentApiUrl}/${id}`, { headers: this.getHeaders() })
      .pipe(catchError((err) => throwError(() => err)));
  }

  updateStudent(id: number, data: Partial<Student>): Observable<Student> {
    return this.http
      .put<Student>(`${this.AdminApiUrl}/${id}`, data, { headers: this.getHeaders() })
      .pipe(catchError((err) => throwError(() => err)));
  }
  ...
  getStudentDetails(studentId: number): Observable<StudentDetail[]> {
    return this.http
      .get<
        StudentDetail[]
      >(`${this.AdminApiUrl}/${studentId}/education`, { headers: this.getHeaders() })
      .pipe(catchError((err) => throwError(() => err)));
  }
  ...
  getProfile(): Observable<any> {
    ...
    return this.http
      .get(`${this.StudentApiUrl}/profile`, {
  ...
  updateProfile(data: Partial<Student>): Observable<any> {
    return this.http
      .put(`${this.StudentApiUrl}/profile`, data, {
        headers: this.getHeaders(),
      })
      .pipe(catchError((err) => throwError(() => err)));
  }
