...
export class McqService {
  ...
  createMcq(mcq: Mcq): Observable<Mcq> {
    return this.http.post<Mcq>(`${this.adminApiUrl}/add`, mcq, { headers: this.getHeaders() }).pipe(
      map((response) => {
  ...
  getAllCourses(): Observable<Course[]> {
    return this.http.get<any>(this.courseApiUrl, { headers: this.getHeaders() }).pipe(
      map((res) => {
  ...
  getCourse(id: number): Observable<Course> {
    return this.http.get<any>(`${this.courseApiUrl}/${id}`, { headers: this.getHeaders() }).pipe(
      map((response) => {
        if (response?.data) {
