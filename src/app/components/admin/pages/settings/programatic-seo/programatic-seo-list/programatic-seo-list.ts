...
export class ProgramaticSeoList implements OnInit, OnDestroy {
  ...
  viewOnWebsite(focusKeyword: string): void {
    const slug = focusKeyword.toLowerCase().replace(/\s+/g, '-');
    window.open(`https://dotbitz.com/seo/${slug}`, '_blank');
  }
  ...
  // viewOnWebsite(focusKeyword: string): void {
//     const slug = focusKeyword.toLowerCase().replace(/\s+/g, '-');
//     window.open(`http://localhost:8000/seo/${slug}`, '_blank');
// }
}