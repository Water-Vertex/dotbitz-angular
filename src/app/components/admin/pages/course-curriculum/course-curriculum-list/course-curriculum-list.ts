...
export class CourseCurriculumList implements OnInit, OnDestroy {
  ...
  // Step 2 — Selected course + its curriculums
...
loadCourses(): void {
  ...
  this.curriculumService.getCourses().subscribe({
    next: (res: any) => {
      ...
      // ✅ Fix thumbnail paths — same as course-list
  ...
  // ===== STEP 2 — Select Course → Load its Curriculum =====
  selectCourse(course: Course): void {
    this.selectedCourse = course;
    this.loadCurriculumsByCourse(course.id!);
  }
  ...
  loadCurriculumsByCourse(courseId: number): void {
    this.isLoading = true;
    this.curriculumService.getCurriculums().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const all = res.data
        ...
        this.curriculums = (all as CourseCurriculum[])
          .filter(c => c.course_id?.toString() === courseId.toString())
  ...
  deleteCurriculum(id: number): void {
    ...
    this.curriculumService.deleteCurriculum(id).subscribe({
      next: () => {
        this.toastService.success('Success', 'Curriculum deleted');
        this.loadCurriculumsByCourse(this.selectedCourse!.id!);
      },
      error: () => { this.toastService.error('Error', 'Delete failed'); }
    });
  }
  ...
  getImageUrl(image: string): string {
    ...
    return `${base}/assets/images/courses/${image}`;
  }
