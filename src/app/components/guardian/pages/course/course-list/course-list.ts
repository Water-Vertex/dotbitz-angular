...
@Component({
  selector: 'app-guardian-course-list',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink],
  templateUrl: './course-list.html',
})
export class GuardianCourseList implements OnInit, OnDestroy {
  courses: Course[] = [];
  ...
  loadCourses(): void {
    ...
    this.courses.forEach((c) => {
      if (c.thumbnail_image && !c.thumbnail_image.startsWith('http')) {
        ...
        c.thumbnail_image = `http://localhost:8000/assets/images/courses/${c.thumbnail_image}`;
