...
//     const percentage = (parseFloat(item.obtain_marks) / parseFloat(item.total_marks)) * 100;
...
export class StudentAssessmentList implements OnInit, OnDestroy {
  ...
  // Course view state
  courses: any[] = [];               // unique courses extracted from assessments
  selectedCourse: any = null;        // currently selected course object
  courseAssessments: any[] = [];     // assessments filtered for selected course
  ...
  // Build unique courses list from assessments
  extractCourses(): void {
    const seen = new Set<number>();
    this.courses = [];
    for (const item of this.assessments) {
      const course = item.assessment_query?.course;
  ...
  selectCourse(course: any): void {
    ...
    this.courseAssessments = this.assessments.filter(
      (item) => item.assessment_query?.course?.id === course.id
    );
    this.searchTerm = '';
    this.cdr.detectChanges();
  }
  ...
  // Filtered course assessments by search
  get filteredCourseAssessments(): any[] {
    if (!this.searchTerm) return this.courseAssessments;
    const t = this.searchTerm.toLowerCase();
    return this.courseAssessments.filter(item =>
      item.assessment?.assessment_title?.toLowerCase().includes(t)
    );
  }
  ...
  onViewAttempt(item: any): void {
    if (!this.canViewAttempt(item)) return;
    this.router.navigate(['/student/assessment/view', item.id]);
  }

  getCompletedCount(): number {
    return this.filteredAssessments.filter(item => item.attempt?.status === 'completed').length;
  }

  getPendingCount(): number {
    return this.filteredAssessments.filter(item => !item.attempt || item.attempt?.status === 'pending').length;
  }
  ...
  getStatusBadgeClass(item: any): string {
    if (!item.attempt) {
      return item.due_date && this.isOverdue(item.due_date) ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600';
    }
    switch (item.attempt?.status) {
      case 'pending': return 'bg-yellow-100 text-yellow-700';
      case 'completed': return 'bg-green-100 text-green-700';
  ...
  getStatusDotClass(item: any): string {
    if (!item.attempt) {
      return item.due_date && this.isOverdue(item.due_date) ? 'bg-red-500' : 'bg-gray-400';
    }
    switch (item.attempt?.status) {
      case 'pending': return 'bg-yellow-500';
      case 'completed': return 'bg-green-500';
  ...
  getStatusText(item: any): string {
    if (!item.attempt) {
      return item.due_date && this.isOverdue(item.due_date) ? 'Overdue' : 'Not Started';
    }
    switch (item.attempt?.status) {
      case 'pending': return 'Pending';
      case 'completed': return 'Completed';
  ...
  canViewAttempt(item: any): boolean {
    return item.attempt?.status === 'completed' || item.attempt?.status === 'time_up';
  }
  ...
  isOverdue(dueDate: string): boolean {
    ...
    return dueDate < `${y}-${m}-${d} ${h}:${mi}:${s}`;
  }

  formatDateOnly(dbDate: string): string {
    if (!dbDate) return '---';
    try {
      const [year, month, day] = dbDate.split(' ')[0].split('-');
