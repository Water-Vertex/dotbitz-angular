...
export class InstructorCourseDetail implements OnInit {
  course: any = null;
  id!: string;
  loading = false;
  ...
  getCourseDetail() {
    ...
    this.courseService.getInstructorCourseDetail(+this.id).subscribe({
      next: (res: any) => {
      ...
      error: (err) => {
        console.error('Instructor Course detail error:', err);
        this.loading = false;
        this.cdr.detectChanges();
      },
    });
