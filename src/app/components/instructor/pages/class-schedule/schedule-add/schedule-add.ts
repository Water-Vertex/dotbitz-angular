...
//            this.addSchedulesToGoogleCalendar(schedules);
//           this.router.navigate(['/instructor/class-schedule/list']);
//         },
//         error: (err: any) => {
...
//   }
//   // Method add karo — same as admin
// private addSchedulesToGoogleCalendar(schedules: any[]): void {
//   const courseName = this.courses.find(c => c.id == schedules[0]?.course_id)?.course_name
//     || 'Class';
//   const batchName = this.batches.find(b => b.id == schedules[0]?.batch_id)?.name
//     || '';

//   const calendarSchedules = schedules.map(s => ({
//     title:       `${courseName} — ${s.day ? s.day.charAt(0).toUpperCase() + s.day.slice(1) : 'Class'}`,
...
// Define interface for calendar schedule
...
export class ClassScheduleAdd implements OnInit {
  ...
  onSubmit(): void {
    ...
    // Explicitly type the days array
    const days: string[] = (baseFormData.days as any[]).map((d: any) => d.day);

    // Explicitly type the schedules array with ScheduleData interface
  ...
  private addSchedulesToGoogleCalendar(schedules: ScheduleData[]): void {
    const courseName = this.courses.find(c => c.id == schedules[0]?.course_id)?.course_name || 'Class';
    const batchName = this.batches.find(b => b.id == schedules[0]?.batch_id)?.name || '';

    const calendarSchedules: CalendarSchedule[] = schedules.map(s => ({
      title: `${courseName} — ${s.day ? s.day.charAt(0).toUpperCase() + s.day.slice(1) : 'Class'}`,
      startTime: s.start_time,
      endTime: s.end_time,
      location: s.meeting_link || '',
